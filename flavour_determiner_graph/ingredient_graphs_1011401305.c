...
// intNode *check_neighbours(intNode *h, int index, int k){
//   if(k==0){
//     return h;
//   }
//   for (int i=0; i<MAT_SIZE; i++)
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
    ...
    for (int i=0; i<MAT_SIZE; i++)
    {
      ...
      {
          h=insertInt(h,i);
          h=related_k_dist(h,ingredients[i],k-1,0);
      }
    }
    return h;
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
     ...
     * 
     * intNode *head=NULL;
     * head=related_with_restrictions("rice", "nuts", 2, 1);
     * 
    ...
    if(k_avoid==0)
    {
        z = ingredient_index(avoid);
        if (z!=-1)
        {
            head2=insertInt(head2, z);
        }
    }
    else{
        head2=related_k_dist(head2, avoid, k_avoid, 0);
        z = ingredient_index(avoid);
        if(z != -1 && searchInt(head2, z)==0)
        {
            head2=insertInt(head2, z);
        }
    }
    ...
    while(temp != NULL)
    {
        if(searchInt(head2, temp->x)==0)
        {
            new = insertInt(new, temp->x);
        }
        temp = temp->next;
    }
    ...
    ******/
    ...
    for(int i=0; i<9; i++)
    {
        ...
        if(strlen(recipe[i]) > 0){
          int cash = ingredient_index(recipe[i]);
          tracker[count] = cash;
          count++;
      ...
      int track = 0;
        ...
        if(track == 0)
        {
            ...
            for(int m=0; m<count; m++){
              cheeseburger += AdjMat[i][tracker[m]];
            }
            if(cheeseburger > higest)
            {
                higest = cheeseburger;
                ingredient = i;
            }
    ...
    strcpy(recipe[index], ingredients[ingredient]);
}
