...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     ****/
    BST_Node *p = NULL;
    p = (BST_Node*) calloc(1, sizeof(BST_Node));
    if (p==NULL)
    {
    printf("new_BST_Node(): Error, there is not enough memory to create new node, returningNULL\n");
    return NULL;
    }
    p->freq = freq;
    p->bar = bar;
...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
    ...
    if(root->key > new_node->key){
        ...
        new_node->index += 0.0001;
        new_node->key = (10.0*(new_node->bar))+(new_node->index);
        root->right = BST_insert(root->right,new_node);
    }
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
     ...
     ****/
     ...
     //if the key is in the current note
     if(root->key == k){
        //case 1 
        ...
        //case 3
            ...
            t = find_successor(root->right);
            root->bar = t->bar;
            root->freq = t->freq;
            root->index = t->index;
            root->key = t->key;
            root->right = BST_delete(root->right,t->bar,t->index);
...
int get_biggest(BST_Node *root){
    if(root == NULL){
        return 0;
    }

    int left = get_biggest(root->left);
    int right = get_biggest(root->right);
    int current = root->bar;
...
int get_smallest(BST_Node *root){
    if(root == NULL){
        return 1000;
    }

    int left = get_smallest(root->left);
    int right = get_smallest(root->right);
    int current = root->bar;
...
BST_Node *invert(int biggest, int smallest, BST_Node *root){
    if(root == NULL){
        return NULL;
    }
    ...
    root->left = invert(biggest,smallest,root->left);
    root->right = invert(biggest,smallest, root->right);
    p = root->left;
    root->left = root->right;
    root->right = p;
    root->bar = biggest+smallest-(root->bar);
...
BST_Node *reverseSong(BST_Node *root)
{
    ...
    smallest_bar = get_smallest(root);
    biggest_bar = get_biggest(root);
    root = invert(biggest_bar, smallest_bar, root);
    return root;
}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    int i = 0;
    while(note_freq[i] != root->freq){
        i++;
    }
    int new_freq = i+semitones;
    double new_index = root->index+time_shift;

    if(new_freq >= 0 && new_freq <= 99 && new_index >= 0 && new_index < 1){
        BST_Node *p = newBST_Node(note_freq[new_freq], root->bar, new_index);
        BST_insert(root, p);
    }
